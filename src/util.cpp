...
const char * const BITCOIN_CONF_FILENAME = "tenebrixcoin.conf";
const char * const BITCOIN_PID_FILENAME = "tenebrixcoind.pid";
...
#else
    const char* pszModule = "tenebrixcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TenebrixCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TenebrixCoin";
#else
    // Unix
    return pathRet / ".tenebrixcoin";
#endif
#endif
}
