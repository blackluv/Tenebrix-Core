...
#if defined(NDEBUG)
# error "TenebrixCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "TenebrixCoin Signed Message:\n";
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // tenebrixcoin: Switch maturity at digishield activation
...
void ThreadScriptCheck() {
    RenameThread("tenebrixcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // tenebrixcoin: BIP30 has been active since inception
    ...
    // tenebrixcoin: BIP16 has been enabled since inception
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // tenebrixcoin: Disallow AuxPow blocks before it is activated.
