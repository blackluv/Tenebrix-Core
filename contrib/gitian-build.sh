...
SIGNER=
VERSION=
url=https://github.com/tenebrixcoin/tenebrixcoin
proc=2
mem=2000
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/tenebrixcoin/tenebrixcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/tenebrixcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/tenebrixcoin/gitian.sigs.git
    git clone https://github.com/tenebrixcoin/tenebrixcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/tenebrixcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean tenebrixcoin git directory because of old caching
    if [ -d inputs/tenebrixcoin/ ]; then
        echo "Cleaning Dogecoin directory..."
        rm -rf inputs/tenebrixcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit tenebrixcoin="$COMMIT" --url tenebrixcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
