...
This tutorial will help you to go through the basics to use Dogecoin Core after you completed the [installation instructions](/INSTALL.md). You now have `tenebrixcoind` or `tenebrixcoin-qt` executables available to run a node, and `tenebrixcoin-cli`/`tenebrixcoin-tx` tools to help you transact DOGE.
...
> ```console
> shibetoshi:~$ /path/to/tenebrixcoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a tenebrixcoin node](#starting-a-tenebrixcoin-node)
...
To start your node, you can run an headless server using `tenebrixcoind`:
```console
shibetoshi:~$ tenebrixcoind -daemon
```

Or you can use the Graphical User Interface (GUI), `tenebrixcoin-qt`:
```console
shibetoshi:~$ tenebrixcoin-qt
```
...
> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `tenebrixcoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `tenebrixcoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.
...
The Dogecoin Core installation provides the `tenebrixcoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ tenebrixcoin-cli help

#Get help for a specific command
shibetoshi:~$ tenebrixcoin-cli help COMMAND
```
...
```console
shibetoshi:~$ tenebrixcoin-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ tenebrixcoin-cli getnewaddress
...
```console
shibetoshi:~$ tenebrixcoin-cli backupwallet /path/of/wallet/backup
```
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli getbalance "*" minconf
```
...
```console
shibetoshi:~$ tenebrixcoin-cli getbalance "*" 5
421.552000
```
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ tenebrixcoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ tenebrixcoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ tenebrixcoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ tenebrixcoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `tenebrixcoin-cli decoderawtransaction` or `tenebrixcoin-tx -json` to convert the content to JSON format.
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ tenebrixcoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ tenebrixcoin-cli sendrawtransaction 
...
shibetoshi:~$ tenebrixcoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ tenebrixcoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ tenebrixcoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ tenebrixcoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `tenebrixcoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ tenebrixcoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `tenebrixcoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.tenebrixcoin` by default, or specify the file location with `-conf`.

**tenebrixcoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/tenebrixcoin.conf).
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `tenebrixcoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `tenebrixcoin-cli`.
...
---------|--------------------
Linux    | `$HOME/.tenebrixcoin`
macOS    | `$HOME/Library/Application Support/Dogecoin`
Windows  | `%APPDATA%\Dogecoin`

You may need to specify `-datadir` also when using `tenebrixcoin-cli`.
