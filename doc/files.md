...
macOS    | `$HOME/Library/Application Support/TenebrixCoin/`
Windows  | `%APPDATA%\Dogecoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `tenebrixcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `tenebrixcoin.conf`       | User-defined configuration settings for `tenebrixcoind` or `tenebrixcoin-qt`; can be specified by `-conf` option
`./`               | `tenebrixcoind.pid`        | Stores the process ID (PID) of `tenebrixcoind` or `tenebrixcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `tenebrixcoind` or `tenebrixcoin-qt`
...
`tenebrixcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
