...
**Optional dependencies** :
```bash
# Qt (required for tenebrixcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `tenebrixcoind`, `tenebrixcoin-cli` and `tenebrixcoin-qt`.
...
#### Enable Qt GUI
Create `tenebrixcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
